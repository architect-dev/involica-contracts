...
describe('InvolicaResolver', function () {
	...
	describe('checkPositionExecutable()', async () => {
		...
		it('should return correct swapsAmountOutMins ETH', async () => {
			...
			const usdcAmounts = await uniRouter.getAmountsOut(defaultEtherDCA, usdcSwapRoute)

			const usdcAmountOutMin: BigNumber = usdcAmounts[usdcAmounts.length - 1]
				.mul(BigNumber.from(10000).sub(defaultSlippage))
				.div(10000)

			const taskData = involica.interface.encodeFunctionData('executeDCA', [bob.address, [usdcAmountOutMin]])
